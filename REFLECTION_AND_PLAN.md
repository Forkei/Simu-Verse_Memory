...
*   [x] **Centralize Configuration:** Move API keys, model names, prompts, and other configurations out of scripts and into dedicated config files (`config.yaml`?) or rely solely on `.env` loaded centrally. (Partially done: API keys, default models use .env. Agent personalities loaded from profiles. Subconscious prompts moved to templates).
