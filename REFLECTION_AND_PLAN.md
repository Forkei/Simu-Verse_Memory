...
*   [x] **Centralize Configuration:** Move API keys, model names, prompts, and other configurations out of scripts and into dedicated config files (`config.yaml`?) or rely solely on `.env` loaded centrally. (Done: API keys, default models use .env. Agent personalities, subconscious prompts, tools, memory categories loaded via paths in `config.yaml`).
...
*   [x] **UI Updates:** Add UI elements (optional) to display retrieved memories or agent reflections derived from memory. (Partially done: thinking indicator, chat history parsing, movement status, chat ordering/timestamps, memory display panel added. Needs agent detail panels).
